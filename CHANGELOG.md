...
- **P14.F — `reprocess articles` chat intent + `intelligence_processed` flag** (`backend/storage/article_store.py`, `backend/api/chat.py`) — `@correspondent reprocess articles` (also matches `refresh articles`, `reclassify articles`, `rerun phase 14`) walks every email/forward source sequentially and pushes existing articles through the Phase 14 pipeline. New `intelligence_processed` column on articles; flips to 1 after successful classify→summary→entity→section→event sequence so re-runs skip already-flagged articles cleanly. Embed pass also gates on existing embedding to save tokens on backfill. Lets users adopt Phase 14 on their historical data without re-ingesting.
- **P14.G — `re-extract all` chat intent** (`backend/api/chat.py`) — one-time backfill: walks every email/forward source, re-runs `extract_articles` with current heuristics, replaces article rows only when new extraction yields MORE articles than current count. Cascade-cleans old `art-` RAG chunks + entity associations before replacing. cnt=0 sources also covered (matches backfill behavior). Single-article-by-structure newsletters (Hustle, personal blogs) stay at 1; multi-section newsletters split.
- **P14.G.2 — Bucketed re-extract diagnostic** — re-extract reply now surfaces the full Correspondent source distribution: 0-article (needs backfill), 1-article (will re-attempt), ≥2-article (already split). Points at the right command for each bucket so users don't have to guess.
- **P14.H / H.2 / H.3 — Backend stability under load** — three layered concurrency fixes after observed crashes during reprocess + IMAP collisions:
  - **P14.H**: `_ARTICLE_PIPELINE_LOCK` + status singleton in `chat.py` covering reprocess + reextract handlers — fast-fail "already running" reply when user fires a second batch.
  - **P14.H.2**: lock moved INSIDE `_summarize_articles_background` itself so every entry point (chat reprocess, chat reextract, IMAP poll fan-out, batch backfill, single-source backfill) serializes through the same FIFO `_PROCESSOR_LOCK`. Closes the IMAP-burst race that was the real crash root cause.
  - **P14.H.3**: per-model semaphores in `ollama_service.py` (gemma4=1 concurrent, phi4-mini=2, embed=4). Caps Ollama queue depth so background fan-out (curator brain, mental_model_inference, etc.) can't collapse it. Also fixed per-article entity extraction from `asyncio.create_task` (fire-and-forget escaped the lock) → `await` (stays inside lock window).
- **P14.RES — Resilience pass (event dedup + IMAP cap + startup defer + background jobs)** —
  - Removed duplicate `source_ingested` emits from `correspondent_processor.ingest_newsletter`/`ingest_forward` — `source_store.create()` already emits as actor='system' and the @correspondent emit was doubling every brain dispatch (mental_model + stance + anticipatory_draft) per newsletter.
  - IMAP per-cycle batch cap of 5 emails (`agents/correspondent.py`) — bursts no longer fan out dozens of background article-processing tasks. Excess queues for next poll cycle.
  - `memory_manager` scheduler sleeps 5 minutes after startup before first iteration — was firing Tier 3 gemma4 work immediately on backend boot, competing with IMAP catch-up.
  - `reprocess articles` + `re-extract all` chat handlers now spawn `asyncio.create_task(_run_*_bg)` and return immediately. Chat connection closes within 1s; work proceeds under the lock. Status visible via `pipeline status` intent. Fixes the "3-min HTTP hang then WebView reload" symptom that looked like a backend crash.
- **P14.MEM — Move background gemma4 callers to phi4-mini** (`backend/services/correspondent_processor.py`) — `classify_email` and link classification flipped to `ollama_fast_model`. IMAP was hitting gemma4 on every newsletter via `classify_email`, keeping the 9.6 GB model pinned in memory permanently via warmup pings. With these flips, gemma4 only loads when user actually chats; idle Ollama footprint drops from ~14 GB to ~4 GB on 18 GB Macs. Gemma4 evicts naturally after 10 min idle.
- **P14.DX — `diagnose extraction` chat intent** (`backend/services/article_extraction_diagnostic.py`) — read-only analyzer. Walks every email/forward source, runs each heuristic independently, captures HTML/text structural markers, classifies each source as "split correctly" / "probable misfire" / "probable genuine single-article". Writes a JSON report to `~/Library/Application Support/LocalBook/diagnostics/article-extraction-<ts>.json`. Chat reply summarizes; full per-source detail in the JSON. **Data-driven extraction improvements depend on running this first.**
- **P14.EXT — Marker-based header split + HTML→text fallback** (`backend/services/article_extractor.py`, `backend/api/chat.py`) — diagnostic revealed TLDR-style newsletters (6 H1s wrapped in nested `<div class="container"><div class="text-block">`) returned 0 from the old sibling-walk headers heuristic, falling back to text_separators (2 articles instead of 6). Rewritten as marker-based: inserts unique boundary string before each target header (prefers H1, falls back to H2/H3), serializes the soup, splits on the marker. Works for both flat and nested layouts. Also added HTML→text fallback in `extract_articles` for Beehiiv-style sources with empty `body_text` but intact HTML. `re-extract all` updated to walk every source and replace only when new extraction yields MORE articles than current count. **Fixture-validated**: TLDR-shape 2→6, Beehiiv-shape 1→4. **User-verified**: fresh IMAP ingests of TLDR newsletters now produce 5-6 articles each.
- **P14.CHATCACHE — Per-notebook chat cache moved to sessionStorage** (`src/components/ChatInterface.tsx`) — user reported chat history vanishing when switching notebooks and returning. Root cause: the old `useRef<Map>` cache lived on the ChatInterface component instance. Different notebooks have different saved canvas layouts in localStorage (single leaf vs split, different leaf IDs), causing React to unmount/remount CanvasPanel on switch and wiping the ref. Cache now lives in `sessionStorage` under key `lb_chat_cache_v1` — survives any remount, auto-clears on app close. Three effects: save outgoing on notebook switch, restore on first mount, continuously persist on every message change.
- **P14.BATCH — Combine classify + summarize + section into one phi4-mini call** (`backend/services/article_batch_processor.py`, `backend/services/article_sectioner.py`, `backend/services/correspondent_processor.py`) — per-article LLM cost was 3 separate phi4-mini calls × ~30s each = ~90s per content article. Combined into ONE phi4-mini JSON call returning `{kind, kind_confidence, summary, topic_tags, match_existing_id, proposed_new_section, section_confidence}`. For non-content articles, the LLM is instructed to leave summary/section fields null, so we still get the kind decision in one call. `_summarize_articles_background_unlocked` rewritten to use the batch when `needs_batch` (no kind classification yet OR no summary). `assign_section` refactored: `apply_section_verdict` extracted as pure persistence so the batch can apply its own verdict without a second LLM call. Standalone `assign_section` retained as fallback. **3x speedup on the per-article pipeline.** **Fixture-validated**: single phi4-mini call returned all four decisions correctly on a Ramp newsletter — kind=content (0.95), section matched existing "AI Accounting" id at 1.0 conf.
