...
XML VALIDITY: `_sw()` takes the fill as a parameter (default `none`) and emits it
ONCE. A filled shape passes its color via `_sw(fill=COLOR)` rather than adding a
separate `fill="…"` attribute — a stray second `fill` makes the whole SVG invalid
XML, which WKWebView rejects when the scene is rendered as an `<img>` data-URI
(the frame shows a broken-image box instead of the drawing).
"""
...
def _sw(n: float = 2.6, fill: str = "none") -> str:
    """Shared stroke attributes. `fill` is emitted here (once) so a caller never
    adds a second `fill` — a duplicate attribute is invalid SVG XML and breaks the
    `<img>` data-URI render in WKWebView."""
    return f'stroke="{INK}" stroke-width="{n}" fill="{fill}" stroke-linecap="round" stroke-linejoin="round"'
...
_FALLBACK = (
    f'<rect x="28" y="28" width="44" height="44" rx="6" {_sw(fill=PAPER)}/>'
)
