...
export function HealthPanel() {
  ...
  return (
    <div className="p-4 space-y-4">
      ...
      {/* Rough edges — recurrence-ranked silent near-misses (quality signals) */}
      <div className="rounded-lg border border-gray-200 dark:border-gray-700 border-l-4 border-l-amber-500 bg-white dark:bg-gray-800 p-4">
        <div className="flex items-center justify-between mb-2">
          <h3 className="text-sm font-semibold text-gray-900 dark:text-white">
            🩹 Rough Edges <span className="text-xs font-normal text-gray-500 dark:text-gray-400">(near-misses, last 7 days)</span>
          </h3>
          <button onClick={refreshSignals} className="px-2 py-1 text-xs text-gray-500 dark:text-gray-400 hover:text-gray-800 dark:hover:text-gray-200">🔄</button>
        </div>
        {signals.length === 0 ? (
          <div className="text-sm text-gray-500 dark:text-gray-400 py-2">✅ No rough edges — clean run. Silent misroutes, engine fallbacks, and empty-retrieval misses show up here when they recur.</div>
        ) : (
          <div className="space-y-1.5">
            {signals.map((g, i) => {
              const sev = g.severity === 'warn' ? 'bg-red-500' : g.severity === 'notable' ? 'bg-amber-500' : 'bg-blue-500';
              return (
                <div key={i} className="rounded-lg bg-gray-50 dark:bg-gray-900/40 px-3 py-2">
                  <div className="flex items-center gap-2">
                    <span className={`w-2 h-2 rounded-full flex-shrink-0 ${sev}`} />
                    <span className="text-xs font-mono px-1.5 py-0.5 rounded bg-gray-200 dark:bg-gray-700 text-gray-700 dark:text-gray-300">{g.type}</span>
                    <span className="text-sm text-gray-800 dark:text-gray-200 truncate">{g.component}{g.key ? ` · ${g.key}` : ''}</span>
                    <span className="ml-auto text-xs font-semibold text-gray-500 dark:text-gray-400 flex-shrink-0">×{g.count}</span>
                  </div>
                  <div className="text-xs text-gray-500 dark:text-gray-400 mt-0.5 pl-4">{g.detail}</div>
                  {g.samples.length > 0 && (
                    <div className="text-[11px] text-gray-400 dark:text-gray-500 mt-0.5 pl-4 italic truncate">e.g. "{g.samples[0]}"</div>
                  )}
                </div>
              );
            })}
