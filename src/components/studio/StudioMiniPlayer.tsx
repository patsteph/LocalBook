    ...
    >
      ...
      <div className="flex items-center justify-between px-3 h-9 bg-gray-50 dark:bg-gray-900 border-b dark:border-gray-700 flex-shrink-0">
        <div className="flex items-center gap-0.5">
          <span className="text-xs font-semibold text-gray-400 uppercase tracking-wider mr-2">Studio</span>
          {TABS.map(tab => (
            <button
              key={tab.id}
              onClick={() => setStudioTab(tab.id)}
              className={`px-2 py-1 rounded-lg transition-colors text-xs hover:bg-gray-200 dark:hover:bg-gray-700 ${
