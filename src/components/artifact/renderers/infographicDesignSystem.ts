/**
 * infographicDesignSystem.ts — the ONE hand-authored L2 lane stylesheet,
 * injected into the Shadow DOM by InfographicArtifactRenderer.
 *
 * MIRROR / SOURCE-OF-TRUTH: byte-for-byte generated from the backend
 * `backend/services/infographic/design_system.py` (`INFOGRAPHIC_L2_CSS`),
 * which the export pipeline injects. Edit the CSS in ONE place and
 * regenerate — do not hand-diverge the two copies. (Same pattern as
 * `htmlArtifactTailwindSubset.ts` <-> `export_assets.TAILWIND_SUBSET_CSS`.)
 */

export const INFOGRAPHIC_L2_CSS = `
/* -- Design tokens ------------------------------------------------ */
.ib{
  --ib-ink:#1b1a18;
  --ib-ink-soft:#4a4844;
