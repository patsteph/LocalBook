...
export const CanvasPanel: React.FC<CanvasPanelProps> = ({ panelId, view, panelProps }) => {
  ...
  const renderContent = () => {
    switch (view) {
      ...
      case 'journey':
        // The spatial Journey Canvas — a top-level view, switched to from Chat
        // (Chat ↔ Canvas) via the panel view switcher. react-flow lazy-loaded.
        return (
          <Suspense fallback={<div className="flex h-full items-center justify-center text-gray-400 text-sm">Loading canvas…</div>}>
